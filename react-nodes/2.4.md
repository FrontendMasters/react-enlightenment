...
React attributes/props not only translate to real HTML attributes props but as previously mentioned can also as configuration values as well as values that are passed child to React element nodes (i.e. React node communication). This aspect of React nodes will be covered in the React props & state chapter.
